...
function ContactForm() {
  ...
  const getMailto = () => {
    ...
    return `mailto:skthunderrobotics@gmail.com?subject=${subject}&body=${body}`
  }
  ...
  return (
    <div>
      ...
      />
      {error && (
        <p style={{ fontSize: '13px', color: '#ff6b6b', marginBottom: '10px' }}>{error}</p>
      )}
      <a
        href={isComplete ? getMailto() : undefined}
        onClick={handleClick}
        style={{
          display: 'block', width: '100%', boxSizing: 'border-box',
          background: isComplete ? '#00b3b3' : '#0e1e1e',
          color: isComplete ? '#05050f' : '#2a4a4a',
          fontFamily: 'Barlow Condensed, sans-serif', fontSize: '16px', fontWeight: 700,
        ...
        onMouseEnter={e => { if (isComplete) e.currentTarget.style.background='#008a8a' }}
        onMouseLeave={e => { if (isComplete) e.currentTarget.style.background='#00b3b3' }}
      >
        Send Message →
      </a>
      <p style={{ fontSize: '12px', color: '#2e2e55', marginTop: '12px', textAlign: 'center', lineHeight: 1.6 }}>
        Clicking send will open your email app with your message ready to go.
      </p>
    </div>
  )
}
